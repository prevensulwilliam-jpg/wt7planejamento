  ...
  return (
    <div className="min-h-screen" style={{ background: '#080C10' }}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <Tabs defaultValue="prevensul" className="w-full">
          <TabsList style={{ background: '#0D1318', border: '1px solid #1A2535' }}>
            <TabsTrigger value="prevensul" className="data-[state=active]:bg-[#1A2535] data-[state=active]:text-[#F0F4F8]" style={{ color: '#94A3B8' }}>
              Comissões Prevensul
            </TabsTrigger>
            {isAdmin && (
              <TabsTrigger value="outros" className="data-[state=active]:bg-[#1A2535] data-[state=active]:text-[#F0F4F8]" style={{ color: '#94A3B8' }}>
                Comissões Outros
              </TabsTrigger>
            )}
          </TabsList>
          <TabsContent value="prevensul">
            <PrevensulTab month={month} userId={userId} />
          </TabsContent>
          {isAdmin && (
            <TabsContent value="outros">
              <OutrosTab month={month} userId={userId} />
            </TabsContent>
          )}
        </Tabs>
      </main>
...
// ═══════════════════════════════════════
//   ABA COMISSÕES OUTROS (ADMIN ONLY)
// ═══════════════════════════════════════

function OutrosTab({ month, userId }: { month: string; userId: string }) {
  return (
    <div className="space-y-6 mt-4">
      <OutrosKPIs month={month} />
      <OutrosForm month={month} userId={userId} />
      <OutrosHistory month={month} />
    </div>
  );
}

function OutrosKPIs({ month }: { month: string }) {
...
function OutrosForm({ month, userId }: { month: string; userId: string }) {
  const { toast } = useToast();
  ...
  return (
    <PremiumCard>
      <h2 className="font-display font-semibold text-lg mb-4" style={{ color: '#F0F4F8' }}>Registrar Comissão Avulsa</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          <label className="text-xs font-mono uppercase mb-1 block" style={{ color: '#94A3B8' }}>Descrição *</label>
          <Input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} style={inputStyle} placeholder="Ex: Comissão venda solar" />
        </div>
        <div>
...
function OutrosHistory({ month }: { month: string }) {
  ...
  if (data.length === 0) return (
    <PremiumCard><p className="text-center py-8 font-mono text-sm" style={{ color: '#4A5568' }}>Nenhuma comissão avulsa neste mês</p></PremiumCard>
  );

  return (
    <PremiumCard>
      <h2 className="font-display font-semibold text-lg mb-4" style={{ color: '#F0F4F8' }}>Histórico — Outras Comissões</h2>
      <div className="overflow-auto rounded-xl" style={{ border: '1px solid #1A2535' }}>
        <Table>
          <TableHeader>
            <TableRow style={{ borderColor: '#1A2535' }}>

...
CREATE TABLE IF NOT EXISTS public.legal_actions (
  ...
  title text NOT NULL,
  description text,
  area text NOT NULL CHECK (area IN ('societario','tributario','familiar','sucessorio','imobiliario','ppci','trabalhista','consumidor','outro')),
  status text NOT NULL DEFAULT 'pendente' CHECK (status IN ('pendente','em_reuniao','contratado','em_execucao','concluido','arquivado','bloqueado')),
  priority text NOT NULL DEFAULT 'media' CHECK (priority IN ('alta','media','baixa')),
  deadline date,
  started_at date,
  completed_at date,
  professional_name text,
  professional_type text CHECK (professional_type IN ('advogado','contador','despachante','cartorio','corretor','outro')),
...
-- Seeds (idempotentes via NOT EXISTS por title)
INSERT INTO public.legal_actions (title, description, area, status, priority, deadline, professional_type, briefing_md, checklist, cost_estimated_min, cost_estimated_max)
SELECT 'T7 Sales — formalização societária com Diego',
  ...
  'societario','pendente','alta','2026-07-01','advogado',
  ...
  2000.00,4000.00
WHERE NOT EXISTS (SELECT 1 FROM public.legal_actions WHERE title='T7 Sales — formalização societária com Diego');

INSERT INTO public.legal_actions (title, description, area, status, priority, deadline, completed_at, professional_type, briefing_md, checklist)
SELECT 'Pacto antenupcial — separação total de bens',
  'Pacto firmado pra casamento 11/12/2027 com a noiva. Regime: separação total de bens.',
  'familiar','concluido','alta','2027-06-01','2026-05-03','advogado',
  'Briefing arquivado em wt7/briefings_juridicos/02_pacto_antenupcial_dez2027.md (RESOLVIDO). Decisão: separação total de bens.',
  jsonb_build_array(
...
WHERE NOT EXISTS (SELECT 1 FROM public.legal_actions WHERE title='Pacto antenupcial — separação total de bens');

INSERT INTO public.legal_actions (title, description, area, status, priority, deadline, professional_type, briefing_md, checklist, cost_estimated_min, cost_estimated_max)
SELECT 'Doação de cotas com usufruto vitalício + janela ITCMD',
  'Estruturar doação de cotas WT7 Holding pra futuros filhos com reserva de usufruto vitalício, antes da reforma federal subir ITCMD de 8% (SC) pra 16-20%.',
  'sucessorio','pendente','media','2026-12-31','advogado',
  ...
  2000.00,5000.00
WHERE NOT EXISTS (SELECT 1 FROM public.legal_actions WHERE title='Doação de cotas com usufruto vitalício + janela ITCMD');

INSERT INTO public.legal_actions (title, description, area, status, priority, deadline, professional_type, briefing_md, checklist, cost_estimated_min, cost_estimated_max)
SELECT 'Estrutura de saque de dividendos T7 + WT7 (escapar retenção 10%)',
  ...
  'tributario','pendente','alta','2026-07-01','contador',
  ...
  1000.00,2000.00
WHERE NOT EXISTS (SELECT 1 FROM public.legal_actions WHERE title='Estrutura de saque de dividendos T7 + WT7 (escapar retenção 10%)');